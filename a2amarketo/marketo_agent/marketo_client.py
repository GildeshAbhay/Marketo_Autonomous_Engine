...
class MarketoClient:
    ...
    def get_campaigns(self, offset: int = 0, max_return: int = 200) -> Dict[str, Any]:
        """Get all campaigns."""
        path = "/rest/asset/v1/smartCampaigns.json"
        params = {"offset": offset, "maxReturn": max_return}
        return self._request("GET", path, params=params)

    def get_smart_lists(self, offset: int = 0, max_return: int = 200) -> Dict[str, Any]:
        """Get all smart lists."""
        path = "/rest/asset/v1/smartLists.json"
        params = {"offset": offset, "maxReturn": max_return}
        return self._request("GET", path, params=params)

    def get_leads(self, filter_type: str = "email", filter_values: list = None, fields: list = None) -> Dict[str, Any]:
        """Get leads by filter."""
        path = "/rest/v1/leads.json"
        params = {
            "filterType": filter_type,
            "filterValues": ",".join(filter_values) if filter_values else "",
            "fields": ",".join(fields) if fields else "id,email,firstName,lastName,company,score"
        }
        return self._request("GET", path, params=params)

    def create_smart_list(self, name: str, description: str, folder_id: int = 1) -> Dict[str, Any]:
